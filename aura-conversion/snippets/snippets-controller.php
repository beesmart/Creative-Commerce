...
class Aura_Conversion_Custom {
	 ...
	 */
	public function __construct( $plugin_name="aura-conversion", $version="1.0.0", $slug=AURA_CONVERSION_SLUG ) {
	...
	public function check_existing_snippets_callback() {
		...
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_CONVERSION_SLUG );
	...
	/**
	 * Display switches for our priv. admins to turn snippets on and off. This is included - rather than calling the parent aura-supercommerces class method, as the ade_ prefix stops the plugins cross contaminating.
	 *
	 * @since 1.0.0
	 * @used-by $this::snippet_form_submission() - this handles the form submission
	 * @see - snippet_form_submit AND associated action - aco_snippet_form_submit
	 * @see   <input name="aco_snippets[]" />
	...
	public function output_snippet_switches(){
		...
		if ($existing_snippets) : ?>
			...
			<div class="snippet-switch row__switch">
				<div class="col__switch">
					...
					<input type="checkbox" id="<?php echo $value->name; ?>" name="aco_snippets[]" value="<?php echo $value->name; ?>" <?php checked( 1 == $value->status ); ?>><span class="checkmark"></span>
					  </label>
				</div>
				<div class="col__switch">
		...
		<?php } ?>
			...
			<input type="hidden" name="action" value="aco_snippet_form_submit">
	...
	/**
	 * handles the submission event for the snippet switches form. This is included - rather than calling the parent aura-supercommerces class method, as the ade_ prefix stops the plugins cross contaminating.
	 *
	 * @since 1.0.0
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=AURA_AGENT
	 * 
	 */

	public function aco_snippet_form_submission(){
		
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_CONVERSION_SLUG );

		$submitted_snippets = false;
		// sanitize the input
		if (isset($_GET['aco_snippets'])) {
			$submitted_snippets = $_GET['aco_snippets'];
		}
		...
		wp_redirect(admin_url('admin.php?page=aura_conversion'));
	...
	*/
	...
	public function aco_refresh_snippets() {
	 ...
	 * @since    1.0.0
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=aura_conversion
	 * @property AURA_CONVERSION_SLUG
	 */


	public function aco_rebuild_snippets() {
