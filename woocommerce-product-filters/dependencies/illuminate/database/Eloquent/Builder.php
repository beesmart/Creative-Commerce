...
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\RecordsNotFoundException;
...
class Builder
{
    ...
    public function firstOrCreate(array $attributes = [], array $values = [])
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->newModelInstance(\array_merge($attributes, $values)), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->firstOrNew($attributes), function ($instance) use($values) {
    ...
    protected function ensureOrderForCursorPagination($shouldReverse = \false)
    {
        ...
        if ($shouldReverse) {
            $this->query->orders = \Barn2\Plugin\WC_Filters\Helpers::collect($this->query->orders)->map(function ($order) {
                $order['direction'] = $order['direction'] === 'asc' ? 'desc' : 'asc';
                return $order;
            })->toArray();
        }
        if ($this->query->unionOrders) {
            return \Barn2\Plugin\WC_Filters\Helpers::collect($this->query->unionOrders);
        }
        return \Barn2\Plugin\WC_Filters\Helpers::collect($this->query->orders);
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->newModelInstance($attributes), function ($instance) {
            $instance->save();
        });
    }
    /**
    ...
    {
        $whereBooleans = \Barn2\Plugin\WC_Filters\Helpers::collect($whereSlice)->pluck('boolean');
