...
trait CanBeOneOfMany
{
    ...
    {
        return $this->ofMany(\Barn2\Plugin\WC_Filters\Helpers::collect(Arr::wrap($column))->mapWithKeys(function ($column) {
            return [$column => 'MAX'];
        })->all(), 'MAX', $relation);
    }
    /**
    ...
    {
        return $this->getRelationName() . '.' . \Barn2\Plugin\WC_Filters\Helpers::last(\explode('.', $column));
    }
    /**
