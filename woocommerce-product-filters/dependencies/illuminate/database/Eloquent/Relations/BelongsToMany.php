...
namespace Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations;
...
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\ModelNotFoundException;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Concerns\AsPivot;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Concerns\InteractsWithDictionary;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Database\Eloquent\Relations\Concerns\InteractsWithPivotTable;
...
class BelongsToMany extends Relation
{
    ...
    protected function resolveTableName($table)
    {
        ...
        if (\in_array(AsPivot::class, \Barn2\Plugin\WC_Filters\Helpers::class_uses_recursive($model))) {
            $this->using($table);
        }
    ...
    protected function aliasedPivotColumns()
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::collect(\array_merge($defaults, $this->pivotColumns))->map(function ($column) {
    ...
    public function paginate($perPage = null, $columns = ['*'], $pageName = 'page', $page = null)
    {
        ...
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->query->paginate($perPage, $columns, $pageName, $page), function ($paginator) {
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->query->simplePaginate($perPage, $columns, $pageName, $page), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        $this->query->addSelect($this->shouldSelect($columns));
        return \Barn2\Plugin\WC_Filters\Helpers::tap($this->query->cursorPaginate($perPage, $columns, $cursorName, $cursor), function ($paginator) {
            $this->hydratePivotRelation($paginator->items());
        });
    }
    /**
    ...
    {
        return Str::camel(Str::pluralStudly(\Barn2\Plugin\WC_Filters\Helpers::class_basename($this->getParent())));
    }
    /**
