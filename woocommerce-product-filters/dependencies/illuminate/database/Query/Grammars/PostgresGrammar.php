...
class PostgresGrammar extends Grammar
{
    ...
    public function whereFullText(Builder $query, $where)
    {
        ...
        $columns = \Barn2\Plugin\WC_Filters\Helpers::collect($where['columns'])->map(function ($column) use($language) {
    ...
    protected function compileUpdateColumns(Builder $query, array $values)
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect($values)->map(function ($value, $key) {
            $column = \Barn2\Plugin\WC_Filters\Helpers::last(\explode('.', $key));
    ...
    public function compileUpsert(Builder $query, array $values, array $uniqueBy, array $update)
    {
        ...
        $columns = \Barn2\Plugin\WC_Filters\Helpers::collect($update)->map(function ($value, $key) {
    ...
    public function compileUpdateFrom(Builder $query, $values)
    {
        ...
        if (isset($query->joins)) {
            ...
            $froms = \Barn2\Plugin\WC_Filters\Helpers::collect($query->joins)->map(function ($join) {
    ...
    {
        $values = \Barn2\Plugin\WC_Filters\Helpers::collect($values)->map(function ($value, $column) {
    ...
    protected function compileUpdateWithJoinsOrLimit(Builder $query, array $values)
    {
        $table = $this->wrapTable($query->from);
        $columns = $this->compileUpdateColumns($query, $values);
        $alias = \Barn2\Plugin\WC_Filters\Helpers::last(\preg_split('/\\s+as\\s+/i', $query->from));
