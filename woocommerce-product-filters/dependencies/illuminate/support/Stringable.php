...
use Closure;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\Conditionable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\Macroable;
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\Tappable;
...
class Stringable implements JsonSerializable
{
    ...
    {
        return new static(\Barn2\Plugin\WC_Filters\Helpers::class_basename($this->value));
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect(\explode($delimiter, $this->value, $limit));
    }
    /**
    ...
    public function split($pattern, $limit = -1, $flags = 0)
    {
        if (\filter_var($pattern, \FILTER_VALIDATE_INT) !== \false) {
            return \Barn2\Plugin\WC_Filters\Helpers::collect(\mb_str_split($this->value, $pattern));
        }
        $segments = \preg_split($pattern, $this->value, $limit, $flags);
        return !empty($segments) ? \Barn2\Plugin\WC_Filters\Helpers::collect($segments) : \Barn2\Plugin\WC_Filters\Helpers::collect();
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect(\sscanf($this->value, $format));
    }
    /**
    ...
    {
        return \Barn2\Plugin\WC_Filters\Helpers::collect(Str::ucsplit($this->value));
    }
    /**
