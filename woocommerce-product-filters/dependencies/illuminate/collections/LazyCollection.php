...
use Barn2\Plugin\WC_Filters\Dependencies\Illuminate\Support\Traits\EnumeratesValues;
...
class LazyCollection implements CanBeEscapedWhenCastToString, Enumerable
{
    ...
    public function last(callable $callback = null, $default = null)
    {
        ...
        return $needle === $placeholder ? \Barn2\Plugin\WC_Filters\Helpers::value($default) : $needle;
    }
    /**
    ...
    public function pluck($value, $key = null)
    {
        return new static(function () use($value, $key) {
            ...
            foreach ($this as $item) {
                $itemValue = \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $value);
                if (\is_null($key)) {
                    (yield $itemValue);
                } else {
                    $itemKey = \Barn2\Plugin\WC_Filters\Helpers::data_get($item, $key);
    ...
    public function sliding($size = 2, $step = 1)
    {
        return new static(function () use($size, $step) {
            ...
            while ($iterator->valid()) {
                ...
                if (\count($chunk) == $size) {
                    (yield \Barn2\Plugin\WC_Filters\Helpers::tap(new static($chunk), function () use(&$chunk, $step) {
