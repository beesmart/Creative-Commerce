...
class Aura_Publicity_Custom {
	 ...
	 */
	public function __construct( $plugin_name="aura-publicity", $version="1.0.0", $slug=AURA_PUBLICITY_SLUG ) {
	...
	public function check_existing_snippets_callback() {
		...
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_PUBLICITY_SLUG );
	 ...
	 * @see - snippet_form_submit AND associated action - apm_snippet_form_submit
	 * @see   <input name="apm_snippets[]" />
	...
	public function output_snippet_switches(){
		...
		if ($existing_snippets) : ?>
			...
			<div class="snippet-switch row__switch">
				<div class="col__switch">
					...
					<input type="checkbox" id="<?php echo $value->name; ?>" name="apm_snippets[]" value="<?php echo $value->name; ?>" <?php checked( 1 == $value->status ); ?>><span class="checkmark"></span>
					  </label>
				</div>
				<div class="col__switch">
		...
		<?php } ?>
			...
			<input type="hidden" name="action" value="apm_snippet_form_submit">
	 ...
	 * @since 1.0.0
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=AURA_PUBLICITY
	 * 
	 */

	public function apm_snippet_form_submission(){
		
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_PUBLICITY_SLUG );

		$submitted_snippets = false;
		// sanitize the input
		if (isset($_GET['apm_snippets'])) {
			$submitted_snippets = $_GET['apm_snippets'];
		}
		...
		wp_redirect(admin_url('admin.php?page=aura_publicity'));
	...
	*/
	...
	public function apm_refresh_snippets() {
	 ...
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=aura_publicity
	 * @property AURA_PUBLICITY_SLUG
	 */


	public function apm_rebuild_snippets() {
