 ...
 * @subpackage Aura_Publicity/admin
 */
...
class Aura_Publicity_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Aura_Publicity_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Aura_Publicity_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/aura-publicity-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/aura-publicity-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function check_required_plugins() {
		...
		if (!function_exists('is_plugin_active') || !is_plugin_active('aura-supercommerce/aura-supercommerce.php')) { 
		    ...
		    deactivate_plugins( '/aura-publicity/aura-publicity.php' );
	...
	public function plugin_activate_admin_notice__fail() {
		...
		$message = __( 'Aura SuperCommerce must be activated to enable this plugin', 'aura_publicity' );
	...
	public function admin_page_display(){ 
	    require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/aura-publicity-admin-display.php';
	}
	 ...
	 *
	 * @since    1.0.0
	 * @see  <input value='apm_rebuild_snippets'>
	 */
		...
		if ( class_exists('Aura_Supercommerce_Admin') ) {
			...
			if ( $show_admin_screens ) {
				...
				<form class="aura-form basic operations" action="<?php echo admin_url('admin-post.php'); ?>" method='post'>
					...
					<input type='hidden' name='action' value='apm_refresh_snippets'>
				...
				<form id="snippet-ops" class="aura-form basic operations" action="<?php echo admin_url('admin-post.php'); ?>" method='post'>
					...
					<input type='hidden' name='action' value='apm_rebuild_snippets'>
				...
				$aura_custom_functions = new Aura_Publicity_Custom( $this->plugin_name, $this->version, AURA_PUBLICITY_SLUG );
