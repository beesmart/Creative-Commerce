...
class Aura_Trade_Booster_Custom {
	 ...
	 */
	public function __construct( $plugin_name="aura-trade-booster", $version="1.0.0", $slug=AURA_TRADE_BOOSTER_SLUG ) {
	...
	public function check_existing_snippets_callback() {
		...
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_TRADE_BOOSTER_SLUG );
	 ...
	 * @see - snippet_form_submit AND associated action - atb_snippet_form_submit
	 * @see   <input name="atb_snippets[]" />
	...
	public function output_snippet_switches(){
		...
		if ($existing_snippets) : ?>
			...
			<div class="snippet-switch row__switch">
				<div class="col__switch">
					...
					<input type="checkbox" id="<?php echo $value->name; ?>" name="atb_snippets[]" value="<?php echo $value->name; ?>" <?php checked( 1 == $value->status ); ?>><span class="checkmark"></span>
					  </label>
				</div>
				<div class="col__switch">
		...
		<?php } ?>
			...
			<input type="hidden" name="action" value="atb_snippet_form_submit">
	 ...
	 * @since 1.0.0
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=aura_trade_booster
	 * 
	 */

	public function atb_snippet_form_submission(){
		
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_TRADE_BOOSTER_SLUG );

		$submitted_snippets = false;
		// sanitize the input
		if (isset($_GET['atb_snippets'])) {
			$submitted_snippets = $_GET['atb_snippets'];
		}
		...
		wp_redirect(admin_url('admin.php?page=aura_trade_booster'));
	...
	*/
	...
	public function atb_refresh_snippets() {
	 ...
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=aura_trade_booster
	 * @property AURA_TRADE_BOOSTER_SLUG
	 */


	public function atb_rebuild_snippets() {
