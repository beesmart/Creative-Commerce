...
function refresh_all_stockist_data(){
    ...
    if ( ! empty( $user_query->get_results() ) ) {
        foreach ( $user_query->get_results() as $user ) {
            ...
            $last_order_id = get_usermeta( $user->ID, 'last_order_id' );

            $address_used = aslc_check_address();

            if ($last_order_id){
                // check if order was deleted or not
                if( get_post_type($last_order_id) == "shop_order" ) : 

                    $compare = aslc_compare_address($items, $address_used, $customer_user_id, $last_order_id, false); 

                endif;

            } else {

                $customer = new WC_Customer( $customer_user_id );
                $last_order = $customer->get_last_order();
                if ($last_order) :
                    $order_id = $last_order->get_id();
                    $date_order = $last_order->get_date_created();
                    $time = strtotime($date_order);

                    if ($time < strtotime('+12 months')) {
						 update_user_meta($customer_user_id, 'user_last_order_date', date('Y-m-d', $time));
						 update_user_meta($customer_user_id, 'last_order_id', $order_id);
                         $compare = aslc_compare_address($items, $address_used, $customer_user_id, $order_id, false); 
    ...
    $url = get_admin_url();
    wp_redirect( $url . '/edit.php?post_type=wpsl_stores&page=wpsl_settings&tab=refresh_settings');
...
//    if (in_array('tradecust', $user_roles)) {
        ...
        $compare = aslc_compare_address($items, $address_used, $customer_user_id, $order_id, true);
//    }
}
...
function aslc_compare_address($items, $address_used, $customer_user_id, $order_id, $is_new) {  
