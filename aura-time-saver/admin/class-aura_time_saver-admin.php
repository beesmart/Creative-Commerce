 ...
 * @subpackage Aura_time_saver/admin
 */
...
class Aura_time_saver_Admin {
	...
	public function enqueue_styles() {
		 ...
		 * defined in Aura_time_saver_Loader as all of the hooks are defined
		 * in that particular class.
		 *
		 * The Aura_time_saver_Loader will then create the relationship
		 * between the defined hooks and the functions defined in this
		 * class.
		 */

		wp_enqueue_style( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'css/aura_time_saver-admin.css', array(), $this->version, 'all' );
	...
	public function enqueue_scripts() {
		...
		wp_enqueue_script( $this->plugin_name, plugin_dir_url( __FILE__ ) . 'js/aura_time_saver-admin.js', array( 'jquery' ), $this->version, false );
	...
	public function check_required_plugins() {
		...
		if (!function_exists('is_plugin_active') || !is_plugin_active('aura-supercommerce/aura-supercommerce.php')) { 
		    ...
		    deactivate_plugins( '/aura-time-saver/aura-time-saver.php' );
	...
	public function plugin_activate_admin_notice__fail() {
		...
		$message = __( 'Aura SuperCommerce must be activated to enable this plugin', 'aura_time_saver' );
	...
	public function admin_page_display(){ 
	    require_once plugin_dir_path( dirname( __FILE__ ) ) . 'admin/partials/aura_time_saver-admin-display.php';
	}
	 ...
	 *
	 * @since    1.0.0
	 * @see  <input value='ats_rebuild_snippets'>
	 */
		...
		if ( class_exists('Aura_Supercommerce_Admin') ) {
			...
			if ( $show_admin_screens ) {
				...
				<form class="aura-form basic operations" action="<?php echo admin_url('admin-post.php'); ?>" method='post'>
					...
					<input type='hidden' name='action' value='ats_refresh_snippets'>
				...
				<form id="snippet-ops" class="aura-form basic operations" action="<?php echo admin_url('admin-post.php'); ?>" method='post'>
					...
					<input type='hidden' name='action' value='ats_rebuild_snippets'>
				...
				$aura_custom_functions = new Aura_time_saver_Custom( $this->plugin_name, $this->version, AURA_TIME_SAVER_SLUG );
