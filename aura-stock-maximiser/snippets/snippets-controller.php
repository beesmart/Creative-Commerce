...
class Aura_Stockist_Custom {
	 ...
	 */
	public function __construct( $plugin_name="aura-stock-maximiser", $version="1.0.0", $slug=AURA_STOCKIST_SLUG ) {
	...
	public function check_existing_snippets_callback() {
		...
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_STOCKIST_SLUG );
	 ...
	 * @see - snippet_form_submit AND associated action - asm_snippet_form_submit
	 * @see   <input name="asm_snippets[]" />
	...
	public function output_snippet_switches(){
		...
		if ($existing_snippets) : ?>
			...
			<div class="snippet-switch row__switch">
				<div class="col__switch">
					...
					<input type="checkbox" id="<?php echo $value->name; ?>" name="asm_snippets[]" value="<?php echo $value->name; ?>" <?php checked( 1 == $value->status ); ?>><span class="checkmark"></span>
					  </label>
				</div>
				<div class="col__switch">
		...
		<?php } ?>
			...
			<input type="hidden" name="action" value="asm_snippet_form_submit">
	 ...
	 * @since 1.0.0
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=AURA_STOCKIST
	 * 
	 */

	public function asm_snippet_form_submission(){
		
		$aura_admin_functions = new Aura_Supercommerce_Custom( $this->plugin_name, $this->version, AURA_STOCKIST_SLUG );

		$submitted_snippets = false;
		// sanitize the input
		if (isset($_GET['asm_snippets'])) {
			$submitted_snippets = $_GET['asm_snippets'];
		}
		...
		wp_redirect(admin_url('admin.php?page=aura_stockist'));
	...
	*/
	...
	public function asm_refresh_snippets() {
	 ...
	 * @FLAG - Migration, migrate
	 * @property wp_redirect - admin.php?page=aura_stockist
	 * @property AURA_STOCKIST_SLUG
	 */


	public function asm_rebuild_snippets() {
